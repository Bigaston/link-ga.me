...
app.post('/submit', function (req, res) {
	...
	db.set("link." + req.body.url, table)
  		.write()
...
app.get("/verify", function(req, res) {
	if (db.has("link." + req.query.link).value()) {
		res.sendStatus(409)
	} else {
		res.sendStatus(200)
	}
...
app.delete("/delete", function(req, res) {
	...
	if (req.body.mail == db.get("link." + req.body.link + ".mail").value().replace("%°", ".")) {
		db.get("link." + req.body.link)
			.remove()
			.write()
		res.sendStatus(401)
	} else {
...
app.get("/usergame", function(req, res) {
	...
	for (i=0; i < user_game.length; i++) {
		response.push(db.get("link." + user_game[i]).value())
	}
...
app.get("/:code", function(req, res) {
	...
	tab = db.get("link." + req.params.code).value()
	var tab2 = {
		title: tab.title, 
		creator: tab.creator, 
